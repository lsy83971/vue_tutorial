...
class reformDict:
    """
    reform a dict
    change the raw pattern of the report
    see `rf_config` for more information.
    """
    @staticmethod
    def setkey(target, keys, value):
        """
        `keys` is a list
        set value to the `keys`-treepath of target
        """
        key = keys[0]
        if len(keys) == 1:
            target[key] = value
    ...
    def reform(self, d):
        """
        1.replace: use d[i] inplace d
        2.delete: del d[i]
        3.reform: set d[*keys] to d[i]
        """
...
class node:
    """
    all the things in each step
    `name`: value from ('MODEL','TYPE1','TYPE2','IDX','CHANNEL')
    `self.ops_next`: a dict that enter a key to find the next node.
    `self.node_next`: find next node
    `self.fit`: initialize
    `self.fit_next`: initialize the next nodes; usually first decide the ops,
    then do the fit for each op.
    """
    ...
    def fit(self, data, order):
        """
        feed the data
        knows the ops of the node
        """
        raise
    def select(self, data, order):
        """
        from the `order`, select the proper part of `data`
        """
        raise

    def fit_next(self, data, order):
        """
        fit the ops for the next step
        """
        pass
...
class OrderRoute():
    ...
    def select(self, order, point='', render=False):
        """
        `render`: bool, if need take out the dataframe or not,
        `point`: nodename of ('MODEL','TYPE1','TYPE2',...)
        if not render return (order,None)
        if render return (order,data)
        """
        ...
        while True:
            ...
            ## finish at this option
            if need_finish:
                return new_order, None

            ## check if the order in the node's ops
            selectactive = True
            for name, ops in ops_dict.items():
                i = choose(order, name)
            ...
            ## if not selectactive return new_order,None
            if selectactive is False:
                return new_order, None

            ## if selectactive copy the choose from original order to new order
            ...
            ## check if we need finish the step
            if point == name:
                need_finish = True
